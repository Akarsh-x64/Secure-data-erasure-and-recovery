...
void TskFileSystem::ProcessMetadataEntry(TSK_FS_FILE*f){++m_status.metadataEntriesSeen;if(!f||!f->meta){++m_status.recordsSkipped;return;}uint64_t id;if(!EnsureRecordForMetadata(f,id)){++m_status.recordsSkipped;return;}auto&i=m_records[m_recordIndexById[id]];PopulateMetadata(i,f);PopulateNamesFromMetadata(i,f);std::vector<Core::DataRange>r;if(ExtractDataRanges(f,r)){i.dataRanges=std::move(r);i.dataRangeStatus=Core::DataRangeStatus::Complete;}else{i.dataRangeStatus=Core::DataRangeStatus::Unavailable;}}
void TskFileSystem::ProcessDirectoryEntry(TSK_FS_FILE*f,const char*p){++m_status.directoryEntriesSeen;if(!f||!f->meta||!f->name||!f->name->name){++m_status.recordsSkipped;return;}std::string n=f->name->name;if(n=="."||n=="..")return;uint64_t id;if(!EnsureRecordForMetadata(f,id)){++m_status.recordsSkipped;return;}auto&i=m_records[m_recordIndexById[id]];PopulateMetadata(i,f);MergeDirectoryName(i,f,p);std::vector<Core::DataRange>r;if(ExtractDataRanges(f,r)){if(i.dataRangeStatus!=Core::DataRangeStatus::Complete||!r.empty())i.dataRanges=std::move(r);i.dataRangeStatus=Core::DataRangeStatus::Complete;}else{i.dataRangeStatus=Core::DataRangeStatus::Unavailable;}}
