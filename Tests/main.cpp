...
public:
  explicit MemoryDiskDevice(size_t totalBytes, uint32_t sectorSize = 512,
                            const std::string &label = "MemoryDisk")
      : m_disk(totalBytes, 0), m_sectorSize(sectorSize), m_label(label) {}

  bool Open(const std::string &) override { return true; }
  void Close() override {}

  bool ReadSectors(uint64_t startSector, uint32_t sectorCount,
                   void *buffer) override {
    size_t startByte = startSector * m_sectorSize;
    size_t byteCount = static_cast<size_t>(sectorCount) * m_sectorSize;
    if (startByte + byteCount > m_disk.size())
      return false;
    std::memcpy(buffer, m_disk.data() + startByte, byteCount);
    return true;
  }

  bool WriteSectors(uint64_t startSector, uint32_t sectorCount,
  ...
  bool SendDeviceCommand(uint32_t, void *, uint32_t, void *,
                         uint32_t) override {
    return true;
  }
...
static void PrintHexDump(const void *data, size_t size,
                         uint64_t basePhysicalOffset,
                         const std::string &label) {
  const auto *bytes = static_cast<const uint8_t *>(data);
  std::cout << "\n-------------------------------------------------------------"
               "-------------------\n";
  std::cout << "[XXD HEX DUMP] " << label << " (" << size
            << " bytes) @ Physical Offset 0x" << std::hex << basePhysicalOffset
            << std::dec << "\n";
  std::cout << "---------------------------------------------------------------"
               "-----------------\n";
...
static void ExplainDataSector(const uint8_t *data, size_t size,
  ...
  if (allZeros) {
    std::cout << "  * Status: ALL 0x00 ZERO-FILLED (Forensically Obliterated / "
                 "Unallocated Free Space)\n";
    std::cout << "  * Forensic Recovery Likelihood: 0.00% (Bit-level zero "
    ...
    // If entropy is high (> 3.5 bits/byte) or uniform distribution, it's PRNG
    // noise / gibberish
    if (entropy > 3.5) {
      std::cout << "  * Status: PRNG GIBBERISH NOISE (DoD 5220.22-M 3-Pass "
                   "Overwritten: 0 -> 1 -> Random)\n";
      std::cout << "  * Shannon Entropy: " << std::fixed << std::setprecision(4)
                << entropy << " bits/byte (High Randomness Noise)\n";
      std::cout << "  * Forensic Recovery Likelihood: 0.00% (Magnetic domains "
                   "randomized by 3-pass sanitization)\n";
    } else {
      std::cout << "  * Status: ACTIVE USER PAYLOAD DATA (Raw disk contents "
                   "present)\n";
...
static void ExplainNtfsBootSector(const uint8_t *sectorData, size_t size,
                                  uint64_t physicalOffset) {
  if (size < sizeof(NTFS::NtfsBootSector))
    return;
  ...
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: NTFS VOLUME BOOT RECORD (VBR) @ 0x"
            << std::hex << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x03..+0x0A [OEM ID]:        \""
            << std::string(vbr->oemId, 8) << "\"\n";
  std::cout << "  * Offset +0x0B..+0x0C [Sector Size]:   "
            << vbr->bytesPerSector << " bytes\n";
  std::cout << "  * Offset +0x0D       [Sec/Cluster]:   "
            << static_cast<int>(vbr->sectorsPerCluster) << " ("
            << (vbr->bytesPerSector * vbr->sectorsPerCluster)
            << " bytes/cluster)\n";
  std::cout << "  * Offset +0x28..+0x2F [Total Sectors]: " << vbr->totalSectors
            << "\n";
  std::cout << "  * Offset +0x30..+0x37 [MFT Start LCN]: Cluster "
            << vbr->mftStartLCN << "\n";
  std::cout << "  * Offset +0x38..+0x3F [MFTMirr LCN]:   Cluster "
            << vbr->mftMirrStartLCN << "\n";
  std::cout << "  * Offset +0x1FE..+0x1FF [Signature]:   0x" << std::hex
            << vbr->bootSignature << std::dec;
  if (vbr->bootSignature == NTFS::NTFS_BOOT_SIGNATURE)
    std::cout << " (Valid 0xAA55 Boot Signature)\n";
  else
...
static void ExplainNtfsMftRecord(const uint8_t *recordData, size_t size,
  ...
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: NTFS MFT RECORD @ 0x" << std::hex
            << physicalOffset << std::dec << "]\n";
  uint32_t magic = hdr->magic;
  std::cout << "  * Offset +0x00..+0x03 [Magic]:         0x" << std::hex
            << magic << std::dec;
  if (magic == NTFS::NTFS_MAGIC_FILE)
    std::cout << " ('FILE' - Active In-Use / Alloc MFT Record)\n";
  else if (magic == NTFS::NTFS_MAGIC_BAAD)
    std::cout << " ('BAAD' - Corrupted / Marked Bad Record)\n";
  else if (magic == 0)
    std::cout << " (0x00000000 - Forensically Zeroed / Obliterated Record)\n";
  else
  ...
  if (hdr->flags & NTFS::FILE_RECORD_IN_USE)
    std::cout << " [IN_USE]";
  if (hdr->flags & NTFS::FILE_RECORD_DIRECTORY)
    std::cout << " [DIRECTORY]";
  if (hdr->flags == 0)
    std::cout << " [UNALLOCATED / FREE RECORD]";
  std::cout << "\n";
  std::cout << "  * Offset +0x18..+0x1B [Used Bytes]:    " << hdr->usedBytes
            << " bytes\n";
  std::cout << "  * Offset +0x1C..+0x1F [Alloc Bytes]:   "
...
static void ExplainXfsSuperblock(const uint8_t *data, size_t size,
                                 uint64_t physicalOffset) {
  if (size < sizeof(XFS::XfsSuperblock))
    return;
  ...
  if (magic == XFS::XFS_SB_MAGIC)
    std::cout << " ('XFSB' - Valid XFS Superblock)\n";
  else
    std::cout << " (Invalid / Sanitized)\n";

  std::cout << "  * Offset +0x04..+0x07 [Block Size]:    "
            << XFS::be32_to_cpu(sb->sb_blocksize) << " bytes\n";
  std::cout << "  * Offset +0x08..+0x0F [Data Blocks]:   "
            << XFS::be64_to_cpu(sb->sb_dblocks) << "\n";
  std::cout << "  * Offset +0x38..+0x3F [Root Inode]:    Inode "
            << XFS::be64_to_cpu(sb->sb_rootino) << "\n";
  std::cout << "  * Offset +0x54..+0x57 [AG Count]:      "
            << XFS::be32_to_cpu(sb->sb_agcount) << "\n";
  std::cout << "  * Offset +0x68..+0x69 [Inode Size]:    "
            << XFS::be16_to_cpu(sb->sb_inodesize) << " bytes\n";
}

static void ExplainXfsInode(const uint8_t *inodeData, size_t size,
                            uint64_t physicalOffset) {
  if (size < sizeof(XFS::XfsDinodeCore))
    return;
  const auto *core = reinterpret_cast<const XFS::XfsDinodeCore *>(inodeData);
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: XFS INODE @ 0x" << std::hex
            << physicalOffset << std::dec << "]\n";
  uint16_t magic = XFS::be16_to_cpu(core->di_magic);
  std::cout << "  * Offset +0x00..+0x01 [Magic]:         0x" << std::hex
            << magic << std::dec;
  if (magic == XFS::XFS_DINODE_MAGIC)
    std::cout << " ('IN' - Active XFS Dinode)\n";
  else if (magic == 0)
    std::cout << " (0x0000 - Sanitized / Forensically Obliterated Inode)\n";
  else
    std::cout << "\n";

  uint16_t mode = XFS::be16_to_cpu(core->di_mode);
  std::cout << "  * Offset +0x02..+0x03 [File Mode]:     0x" << std::hex << mode
            << std::dec;
  if ((mode & XFS::XFS_S_IFMT) == XFS::XFS_S_IFDIR)
    std::cout << " (Directory)\n";
  else if ((mode & XFS::XFS_S_IFMT) == XFS::XFS_S_IFREG)
    std::cout << " (Regular File)\n";
  else if (mode == 0)
    std::cout << " (0x0000 - Free / Obliterated)\n";
  else
    std::cout << "\n";

  std::cout << "  * Offset +0x04       [Version]:       "
            << static_cast<int>(core->di_version) << "\n";
  std::cout << "  * Offset +0x05       [Format]:        "
            << static_cast<int>(core->di_format) << " (";
  if (core->di_format == XFS::XFS_DINODE_FMT_LOCAL)
    std::cout << "Shortform / Local Inline Fork)\n";
  else if (core->di_format == XFS::XFS_DINODE_FMT_EXTENTS)
    std::cout << "Direct Extents Array)\n";
  else if (core->di_format == XFS::XFS_DINODE_FMT_BTREE)
    std::cout << "Multi-Level B+Tree Root Fork)\n";
  else
    std::cout << "Other)\n";

  std::cout << "  * Offset +0x08..+0x0F [File Size]:     "
            << XFS::be64_to_cpu(core->di_size) << " bytes\n";
  std::cout << "  * Offset +0x14..+0x17 [Extent Count]:  "
            << XFS::be32_to_cpu(core->di_nextents) << "\n";
}

static void ExplainExt4Superblock(const uint8_t *data, size_t size,
                                  uint64_t physicalOffset) {
  if (size < sizeof(Ext4::Ext4Superblock))
    return;
  const auto *sb = reinterpret_cast<const Ext4::Ext4Superblock *>(data);
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: ext4 SUPERBLOCK @ 0x" << std::hex
            << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x38..+0x39 [Magic]:         0x" << std::hex
            << sb->s_magic << std::dec;
  if (sb->s_magic == Ext4::EXT4_SUPER_MAGIC)
    std::cout << " (0xEF53 - Valid ext4 Magic)\n";
  else
    std::cout << " (Invalid / Sanitized)\n";

  std::cout << "  * Offset +0x00..+0x03 [Total Inodes]:  " << sb->s_inodes_count
            << "\n";
  std::cout << "  * Offset +0x04..+0x07 [Total Blocks]:  "
            << sb->s_blocks_count_lo << "\n";
  std::cout << "  * Offset +0x18..+0x1B [Block Size]:    "
            << (1024 << sb->s_log_block_size) << " bytes\n";
  std::cout << "  * Offset +0x58..+0x67 [Volume Name]:   \""
            << std::string(sb->s_volume_name, 16) << "\"\n";
}

static void ExplainExt4Inode(const uint8_t *inodeData, size_t size,
                             uint64_t physicalOffset) {
  if (size < sizeof(Ext4::Ext4Inode))
    return;
  const auto *ino = reinterpret_cast<const Ext4::Ext4Inode *>(inodeData);
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: ext4 INODE @ 0x" << std::hex
            << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x00..+0x01 [File Mode]:     0x" << std::hex
            << ino->i_mode << std::dec;
  if ((ino->i_mode & Ext4::EXT4_S_IFMT) == Ext4::EXT4_S_IFDIR)
    std::cout << " (Directory)\n";
  else if ((ino->i_mode & Ext4::EXT4_S_IFMT) == Ext4::EXT4_S_IFREG)
    std::cout << " (Regular File)\n";
  ...
  std::cout << "  * Offset +0x04..+0x07 [File Size]:     " << ino->i_size_lo
            << " bytes\n";
  ...
  if (ino->i_flags & Ext4::EXT4_INLINE_DATA_FL) {
    ...
    const auto *eh =
        reinterpret_cast<const Ext4::Ext4ExtentHeader *>(ino->i_block);
...
static void ExplainExFatBootSector(const uint8_t *data, size_t size,
                                   uint64_t physicalOffset) {
  if (size < sizeof(ExFatBootSector))
    return;
  const auto *vbr = reinterpret_cast<const ExFatBootSector *>(data);
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: exFAT VOLUME BOOT RECORD (VBR) @ 0x"
            << std::hex << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x03..+0x0A [FS Name]:       \""
            << std::string(vbr->fileSystemName, 8) << "\"\n";
  std::cout << "  * Offset +0x6C       [Sec Shift]:     "
            << static_cast<int>(vbr->bytesPerSectorShift) << " ("
            << (1 << vbr->bytesPerSectorShift) << " bytes/sec)\n";
  std::cout << "  * Offset +0x6D       [Clust Shift]:   "
            << static_cast<int>(vbr->sectorsPerClusterShift) << " ("
            << (1 << vbr->sectorsPerClusterShift) << " sectors/cluster)\n";
  std::cout << "  * Offset +0x58..+0x5B [Heap Offset]:   Sector "
            << vbr->clusterHeapOffsetSectors << "\n";
  std::cout << "  * Offset +0x60..+0x63 [RootDir Clust]: Cluster "
            << vbr->rootDirectoryFirstCluster << "\n";
  std::cout << "  * Offset +0x1FE..+0x1FF [Signature]:   0x" << std::hex
            << vbr->bootSignature << std::dec;
  if (vbr->bootSignature == 0xAA55)
    std::cout << " (Valid 0xAA55 Signature)\n";
  else
    std::cout << " (Invalid / Sanitized)\n";
}

static void ExplainExFatDirectoryEntry(const uint8_t *entryData, size_t size,
                                       uint64_t physicalOffset) {
  if (size < 32)
    return;
  uint8_t type = entryData[0];
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: exFAT DIRECTORY ENTRY @ 0x"
            << std::hex << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x00 [Entry Type]:           0x" << std::hex
            << static_cast<int>(type) << std::dec;
  if (type == 0x85) {
    std::cout << " (File Directory Entry)\n";
    if (size >= sizeof(ExFatFileDirectoryEntry)) {
      const auto *fe =
          reinterpret_cast<const ExFatFileDirectoryEntry *>(entryData);
      std::cout << "  * Offset +0x01 [Secondary Count]:      "
                << static_cast<int>(fe->secondaryCount) << "\n";
      std::cout << "  * Offset +0x02..+0x03 [Set Checksum]:  0x" << std::hex
                << fe->setChecksum << std::dec;
      if (fe->setChecksum != 0)
        std::cout << " (Valid Entry Set Checksum)\n";
      else
        std::cout << " (0x0000 - Sanitized / Uncalculated)\n";
      std::cout << "  * Offset +0x04..+0x05 [File Attributes]:0x" << std::hex
                << fe->fileAttributes << std::dec << "\n";
    }
  } else if (type == 0xC0) {
    std::cout << " (Stream Extension Directory Entry)\n";
    if (size >= sizeof(ExFatStreamExtensionDirectoryEntry)) {
      const auto *se =
          reinterpret_cast<const ExFatStreamExtensionDirectoryEntry *>(
              entryData);
      std::cout << "  * Offset +0x03 [Name Length]:          "
                << static_cast<int>(se->nameLength) << " chars\n";
      std::cout << "  * Offset +0x04..+0x05 [Name Hash]:     0x" << std::hex
                << se->nameHash << std::dec << "\n";
      std::cout << "  * Offset +0x14..+0x17 [First Cluster]: Cluster "
                << se->firstCluster << "\n";
      std::cout << "  * Offset +0x18..+0x1F [Data Length]:   " << se->dataLength
                << " bytes\n";
    }
  } else if (type == 0xC1) {
...
static void ExplainFat32BootSector(const uint8_t *data, size_t size,
                                   uint64_t physicalOffset) {
  if (size < sizeof(FAT32::Fat32BootSector))
    return;
  const auto *bpb = reinterpret_cast<const FAT32::Fat32BootSector *>(data);
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: FAT32 VOLUME BOOT RECORD (VBR) @ 0x"
            << std::hex << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x03..+0x0A [OEM Name]:      \""
            << std::string(bpb->oemName, 8) << "\"\n";
  std::cout << "  * Offset +0x0B..+0x0C [Bytes/Sector]:  "
            << bpb->bytesPerSector << " bytes\n";
  std::cout << "  * Offset +0x0D       [Sec/Cluster]:   "
            << static_cast<int>(bpb->sectorsPerCluster) << " ("
            << (bpb->bytesPerSector * bpb->sectorsPerCluster)
            << " bytes/cluster)\n";
  std::cout << "  * Offset +0x0E..+0x0F [Reserved Secs]: "
            << bpb->reservedSectorCount << "\n";
  std::cout << "  * Offset +0x10       [Number of FATs]:"
            << static_cast<int>(bpb->numFATs) << "\n";
  std::cout << "  * Offset +0x24..+0x27 [FAT Size 32]:   " << bpb->fatSize32
            << " sectors\n";
  std::cout << "  * Offset +0x2C..+0x2F [Root Dir Clust]:Cluster "
            << bpb->rootCluster << "\n";
  std::cout << "  * Offset +0x52..+0x59 [FS Type]:       \""
            << std::string(bpb->fileSystemType, 8) << "\"\n";
  std::cout << "  * Offset +0x1FE..+0x1FF [Signature]:   0x" << std::hex
            << bpb->signature << std::dec;
...
static void ExplainFat32DirectoryEntry(const uint8_t *entryData, size_t size,
                                       uint64_t physicalOffset) {
  if (size < sizeof(FAT32::Fat32DirEntry))
    return;
  const auto *sfn = reinterpret_cast<const FAT32::Fat32DirEntry *>(entryData);
  std::cout << "\n[SEMANTIC BYTE BREAKDOWN: FAT32 DIRECTORY ENTRY @ 0x"
            << std::hex << physicalOffset << std::dec << "]\n";
  std::cout << "  * Offset +0x00..+0x0A [Name]:          \""
            << std::string(reinterpret_cast<const char *>(sfn->name), 11)
            << "\"";
  if (sfn->name[0] == FAT32::FAT32_DIR_ENTRY_DELETED)
    std::cout << " (0xE5 - Deleted / Sanitized Entry)\n";
  else if (sfn->name[0] == 0x00)
    std::cout << " (0x00 - Free / End of Directory)\n";
  else
    std::cout << "\n";
  std::cout << "  * Offset +0x0B       [Attributes]:    0x" << std::hex
            << static_cast<int>(sfn->attr) << std::dec;
  if (sfn->attr & FAT32::FAT32_ATTR_DIRECTORY)
    std::cout << " (Directory)\n";
  else
    std::cout << " (File)\n";
  uint32_t cluster =
      (static_cast<uint32_t>(sfn->fstClusHI) << 16) | sfn->fstClusLO;
  std::cout << "  * Offset +0x14 & 0x1A[First Cluster]: Cluster " << cluster
            << "\n";
  std::cout << "  * Offset +0x1C..+0x1F [File Size]:     " << sfn->fileSize
            << " bytes\n";
}
...
static void SetupNtfsSyntheticDisk(MemoryDiskDevice &dev) {
  ...
  *reinterpret_cast<uint32_t *>(a0 + ah0->length) = NTFS::ATTR_END;
  ...
  hdr6->magic = NTFS::NTFS_MAGIC_FILE;
  hdr6->updateSequenceOffset = 0x30;
  hdr6->updateSequenceSize = 3;
  hdr6->sequenceNumber = 1;
  hdr6->firstAttributeOffset = 0x38;
  hdr6->flags = NTFS::FILE_RECORD_IN_USE;
  hdr6->allocatedBytes = 1024;
  hdr6->recordNumber = 6;
  uint8_t *a6 = mft6.data() + 0x38;
  auto *ah6 = reinterpret_cast<NTFS::NtfsAttributeHeader *>(a6);
  ah6->type = NTFS::ATTR_DATA;
  ah6->length = 0x48;
  ah6->nonResidentFlag = 1;
  auto *nrh6 = reinterpret_cast<NTFS::NtfsNonResidentAttributeHeader *>(
      a6 + sizeof(NTFS::NtfsAttributeHeader));
  nrh6->dataRunsOffset = sizeof(NTFS::NtfsAttributeHeader) +
                         sizeof(NTFS::NtfsNonResidentAttributeHeader);
  ...
  *reinterpret_cast<uint32_t *>(a6 + ah6->length) = NTFS::ATTR_END;
  ...
  ah16_fn->type = NTFS::ATTR_FILE_NAME;
  ah16_fn->length = 0x68;
  ah16_fn->nonResidentFlag = 0;
  auto *res16_fn = reinterpret_cast<NTFS::NtfsResidentAttributeHeader *>(
      a16 + sizeof(NTFS::NtfsAttributeHeader));
  res16_fn->valueOffset = sizeof(NTFS::NtfsAttributeHeader) +
                          sizeof(NTFS::NtfsResidentAttributeHeader);
  res16_fn->valueLength = 0x44;
  auto *fn16 = reinterpret_cast<NTFS::NtfsFileNameAttribute *>(
      a16 + res16_fn->valueOffset);
  fn16->parentDirectory = NTFS::MFT_REC_ROOT;
  fn16->realSize = 64;
  ...
  for (size_t i = 0; i < fName16.length(); ++i)
    fn16->fileName[i] = fName16[i];

  uint8_t *a16_d = a16 + ah16_fn->length;
  auto *ah16_d = reinterpret_cast<NTFS::NtfsAttributeHeader *>(a16_d);
  ah16_d->type = NTFS::ATTR_DATA;
  ah16_d->length = 0x48;
  ah16_d->nonResidentFlag = 1;
  auto *nrh16 = reinterpret_cast<NTFS::NtfsNonResidentAttributeHeader *>(
      a16_d + sizeof(NTFS::NtfsAttributeHeader));
  nrh16->dataRunsOffset = sizeof(NTFS::NtfsAttributeHeader) +
                          sizeof(NTFS::NtfsNonResidentAttributeHeader);
  ...
  *reinterpret_cast<uint32_t *>(a16_d + ah16_d->length) = NTFS::ATTR_END;
  ...
  hdr17->magic = NTFS::NTFS_MAGIC_FILE;
  ...
  ah17_fn->type = NTFS::ATTR_FILE_NAME;
  ah17_fn->length = 0x60;
  ah17_fn->nonResidentFlag = 0;
  auto *res17_fn = reinterpret_cast<NTFS::NtfsResidentAttributeHeader *>(
      a17 + sizeof(NTFS::NtfsAttributeHeader));
  res17_fn->valueOffset = sizeof(NTFS::NtfsAttributeHeader) +
                          sizeof(NTFS::NtfsResidentAttributeHeader);
  ...
  for (size_t i = 0; i < fName17.length(); ++i)
    fn17->fileName[i] = fName17[i];

  uint8_t *a17_ir = a17 + ah17_fn->length;
  auto *ah17_ir = reinterpret_cast<NTFS::NtfsAttributeHeader *>(a17_ir);
  ah17_ir->type = NTFS::ATTR_INDEX_ROOT;
  ah17_ir->nonResidentFlag = 0;
  auto *res17_ir = reinterpret_cast<NTFS::NtfsResidentAttributeHeader *>(
      a17_ir + sizeof(NTFS::NtfsAttributeHeader));
  res17_ir->valueOffset = sizeof(NTFS::NtfsAttributeHeader) +
                          sizeof(NTFS::NtfsResidentAttributeHeader);
  auto *ir17 = reinterpret_cast<NTFS::NtfsIndexRootHeader *>(
      a17_ir + res17_ir->valueOffset);
  ir17->attributeType = NTFS::ATTR_FILE_NAME;
  ...
  auto *fn18_idx = reinterpret_cast<NTFS::NtfsFileNameAttribute *>(
      e18Ptr + sizeof(NTFS::NtfsIndexEntry));
  fn18_idx->parentDirectory = 17;
  std::u16string fName18 = u"financials.xlsx";
  fn18_idx->fileNameLength = static_cast<uint8_t>(fName18.length());
  for (size_t i = 0; i < fName18.length(); ++i)
    fn18_idx->fileName[i] = fName18[i];
  e18->keyLength =
      sizeof(NTFS::NtfsFileNameAttribute) + (fn18_idx->fileNameLength * 2);
  e18->length = sizeof(NTFS::NtfsIndexEntry) + e18->keyLength;

  auto *e17_end =
      reinterpret_cast<NTFS::NtfsIndexEntry *>(e18Ptr + e18->length);
  e17_end->length = sizeof(NTFS::NtfsIndexEntry);
  e17_end->flags = NTFS::INDEX_ENTRY_LAST;

  idxHdr17->totalEntriesSize =
      sizeof(NTFS::NtfsIndexHeader) + e18->length + e17_end->length;
  idxHdr17->allocatedSize = idxHdr17->totalEntriesSize;
  res17_ir->valueLength =
      sizeof(NTFS::NtfsIndexRootHeader) + idxHdr17->totalEntriesSize;
  ah17_ir->length = sizeof(NTFS::NtfsAttributeHeader) +
                    sizeof(NTFS::NtfsResidentAttributeHeader) +
                    res17_ir->valueLength;
  *reinterpret_cast<uint32_t *>(a17_ir + ah17_ir->length) = NTFS::ATTR_END;
  ...
  ah18_fn->type = NTFS::ATTR_FILE_NAME;
  ah18_fn->length = 0x68;
  ah18_fn->nonResidentFlag = 0;
  auto *res18_fn = reinterpret_cast<NTFS::NtfsResidentAttributeHeader *>(
      a18 + sizeof(NTFS::NtfsAttributeHeader));
  res18_fn->valueOffset = sizeof(NTFS::NtfsAttributeHeader) +
                          sizeof(NTFS::NtfsResidentAttributeHeader);
  auto *fn18 = reinterpret_cast<NTFS::NtfsFileNameAttribute *>(
      a18 + res18_fn->valueOffset);
  fn18->parentDirectory = 17;
  fn18->fileNameLength = static_cast<uint8_t>(fName18.length());
  for (size_t i = 0; i < fName18.length(); ++i)
    fn18->fileName[i] = fName18[i];

  uint8_t *a18_d = a18 + ah18_fn->length;
  auto *ah18_d = reinterpret_cast<NTFS::NtfsAttributeHeader *>(a18_d);
  ah18_d->type = NTFS::ATTR_DATA;
  ah18_d->length = 0x48;
  ah18_d->nonResidentFlag = 1;
  auto *nrh18 = reinterpret_cast<NTFS::NtfsNonResidentAttributeHeader *>(
      a18_d + sizeof(NTFS::NtfsAttributeHeader));
  nrh18->dataRunsOffset = sizeof(NTFS::NtfsAttributeHeader) +
                          sizeof(NTFS::NtfsNonResidentAttributeHeader);
  ...
  *reinterpret_cast<uint32_t *>(a18_d + ah18_d->length) = NTFS::ATTR_END;
  ...
  hdr5->magic = NTFS::NTFS_MAGIC_FILE;
  ...
  ah5_ir->type = NTFS::ATTR_INDEX_ROOT;
  ah5_ir->nonResidentFlag = 0;
  auto *res5_ir = reinterpret_cast<NTFS::NtfsResidentAttributeHeader *>(
      a5 + sizeof(NTFS::NtfsAttributeHeader));
  res5_ir->valueOffset = sizeof(NTFS::NtfsAttributeHeader) +
                         sizeof(NTFS::NtfsResidentAttributeHeader);
  auto *ir5 =
      reinterpret_cast<NTFS::NtfsIndexRootHeader *>(a5 + res5_ir->valueOffset);
  ir5->attributeType = NTFS::ATTR_FILE_NAME;
  ir5->collationRule = 1;
  ...
  e17_root->keyLength =
      sizeof(NTFS::NtfsFileNameAttribute) + (fn17_root->fileNameLength * 2);
  ...
  auto *fn16_root = reinterpret_cast<NTFS::NtfsFileNameAttribute *>(
      e16Ptr + sizeof(NTFS::NtfsIndexEntry));
  fn16_root->parentDirectory = 5;
  fn16_root->fileNameLength = static_cast<uint8_t>(fName16.length());
  for (size_t i = 0; i < fName16.length(); ++i)
    fn16_root->fileName[i] = fName16[i];
  e16_root->keyLength =
      sizeof(NTFS::NtfsFileNameAttribute) + (fn16_root->fileNameLength * 2);
  e16_root->length = sizeof(NTFS::NtfsIndexEntry) + e16_root->keyLength;

  auto *e_end =
      reinterpret_cast<NTFS::NtfsIndexEntry *>(e16Ptr + e16_root->length);
  e_end->length = sizeof(NTFS::NtfsIndexEntry);
  e_end->flags = NTFS::INDEX_ENTRY_LAST;

  idxHdr5->totalEntriesSize = sizeof(NTFS::NtfsIndexHeader) + e17_root->length +
                              e16_root->length + e_end->length;
  idxHdr5->allocatedSize = idxHdr5->totalEntriesSize;
  res5_ir->valueLength =
      sizeof(NTFS::NtfsIndexRootHeader) + idxHdr5->totalEntriesSize;
  ah5_ir->length = sizeof(NTFS::NtfsAttributeHeader) +
                   sizeof(NTFS::NtfsResidentAttributeHeader) +
                   res5_ir->valueLength;
...
static void SetupXfsSyntheticDisk(MemoryDiskDevice &dev) {
  ...
  *reinterpret_cast<uint16_t *>(rootPtr + 0) =
      XFS::cpu_to_be16(XFS::XFS_DINODE_MAGIC);
  *reinterpret_cast<uint16_t *>(rootPtr + 2) =
      XFS::cpu_to_be16(XFS::XFS_S_IFDIR | 0755);
  rootPtr[4] = 2;
  rootPtr[5] = XFS::XFS_DINODE_FMT_LOCAL;
  ...
  *reinterpret_cast<uint16_t *>(f1Ptr + 0) =
      XFS::cpu_to_be16(XFS::XFS_DINODE_MAGIC);
  *reinterpret_cast<uint16_t *>(f1Ptr + 2) =
      XFS::cpu_to_be16(XFS::XFS_S_IFREG | 0644);
  f1Ptr[4] = 2;
  f1Ptr[5] = XFS::XFS_DINODE_FMT_EXTENTS;
  ...
  *reinterpret_cast<uint32_t *>(f1Ptr + 76) = XFS::cpu_to_be32(1);
  ...
  ext1.l0 = XFS::cpu_to_be64(w0);
  ext1.l1 = XFS::cpu_to_be64(w1);
  ...
  *reinterpret_cast<uint16_t *>(f2Ptr + 0) =
      XFS::cpu_to_be16(XFS::XFS_DINODE_MAGIC);
  *reinterpret_cast<uint16_t *>(f2Ptr + 2) =
      XFS::cpu_to_be16(XFS::XFS_S_IFREG | 0644);
  f2Ptr[4] = 2;
  f2Ptr[5] = XFS::XFS_DINODE_FMT_BTREE;
  ...
  *reinterpret_cast<uint16_t *>(dirPtr + 0) =
      XFS::cpu_to_be16(XFS::XFS_DINODE_MAGIC);
  *reinterpret_cast<uint16_t *>(dirPtr + 2) =
      XFS::cpu_to_be16(XFS::XFS_S_IFDIR | 0755);
  dirPtr[4] = 2;
  dirPtr[5] = XFS::XFS_DINODE_FMT_LOCAL;
  ...
  *reinterpret_cast<uint16_t *>(f3Ptr + 0) =
      XFS::cpu_to_be16(XFS::XFS_DINODE_MAGIC);
  *reinterpret_cast<uint16_t *>(f3Ptr + 2) =
      XFS::cpu_to_be16(XFS::XFS_S_IFREG | 0644);
  f3Ptr[4] = 2;
  f3Ptr[5] = XFS::XFS_DINODE_FMT_EXTENTS;
  ...
  *reinterpret_cast<uint32_t *>(f3Ptr + 76) = XFS::cpu_to_be32(1);
  ...
  ext3.l0 = XFS::cpu_to_be64(ew0);
  ext3.l1 = XFS::cpu_to_be64(ew1);
...
static void SetupExt4SyntheticDisk(MemoryDiskDevice &dev) {
  ...
  sb.s_feature_incompat = Ext4::EXT4_FEATURE_INCOMPAT_FILETYPE |
                          Ext4::EXT4_FEATURE_INCOMPAT_EXTENTS;
  ...
  blockBitmap[0] = 0xFF;
  blockBitmap[1] = 0x03; // Blocks 0..9 allocated

  uint8_t *inodeBitmap = disk + 3 * BLOCK_SIZE;
  inodeBitmap[0] = 0xFF;
  inodeBitmap[1] = 0x1F; // Inodes 1..13 allocated
  ...
  eh->eh_magic = Ext4::EXT4_EXTENT_MAGIC;
  eh->eh_entries = 1;
  eh->eh_max = 4;
  ...
  d3->rec_len = 32;
  d3->name_len = 17;
  d3->file_type = Ext4::EXT4_FT_REG_FILE;
  ...
  file12.i_flags = Ext4::EXT4_EXTENTS_FL;
  auto *eh12 = reinterpret_cast<Ext4::Ext4ExtentHeader *>(file12.i_block);
  eh12->eh_magic = Ext4::EXT4_EXTENT_MAGIC;
  eh12->eh_entries = 1;
  eh12->eh_max = 4;
  ...
  dir13.i_flags = Ext4::EXT4_EXTENTS_FL;
  auto *eh13 = reinterpret_cast<Ext4::Ext4ExtentHeader *>(dir13.i_block);
  eh13->eh_magic = Ext4::EXT4_EXTENT_MAGIC;
  eh13->eh_entries = 1;
  eh13->eh_max = 4;
  ...
  v_d1->rec_len = 12;
  v_d1->name_len = 1;
  v_d1->file_type = Ext4::EXT4_FT_DIR;
  v_d1->name[0] = '.';
  auto *v_d2 = reinterpret_cast<Ext4::Ext4DirEntry2 *>(dirBlock9 + 12);
  v_d2->inode = 2;
  v_d2->rec_len = 12;
  v_d2->name_len = 2;
  v_d2->file_type = Ext4::EXT4_FT_DIR;
  v_d2->name[0] = '.';
  v_d2->name[1] = '.';
  ...
  v_d3->name_len = 8;
  v_d3->file_type = Ext4::EXT4_FT_REG_FILE;
...
static void SetupExFatSyntheticDisk(MemoryDiskDevice &dev) {
  ...
  auto *sEntry =
      reinterpret_cast<ExFatStreamExtensionDirectoryEntry *>(rootDir + 64);
  sEntry->entryType = 0xC0;
  sEntry->firstCluster = 4;
  sEntry->dataLength = 64;
  sEntry->validDataLength = 64;
  sEntry->generalSecondaryFlags =
      0x03; // AllocationPossible (0x01) | NoFatChain (0x02)
  ...
  for (size_t i = 0; i < name.length(); ++i)
    fnEntry->fileName[i] = name[i];

  // Compute and set 16-bit Directory Entry Set Checksum across entries 0x85,
  // 0xC0, 0xC1 (3 entries)
  fEntry->setChecksum = ExFatDriver::ComputeEntrySetChecksum(
      reinterpret_cast<const uint8_t *>(fEntry), 3);

  // Populate Sector 11 with the 32-bit Main Boot Checksum repeated across all
  // 512 bytes
  uint32_t bootChecksum =
      ExFatDriver::ComputeBootChecksum(disk, 11 * SECTOR_SIZE);
  uint32_t *csumSector =
      reinterpret_cast<uint32_t *>(disk + (11 * SECTOR_SIZE));
  ...
  std::memcpy(payload, "EXFAT_SUPER_SECRET_PAYLOAD_CONFIDENTIAL_REPORT_2026",
              51);
}
...
static void SetupFat32SyntheticDisk(MemoryDiskDevice &dev) {
  ...
  auto writeFat = [&](uint8_t fatIndex) {
    uint32_t *fat = reinterpret_cast<uint32_t *>(
        disk + ((32 + fatIndex * 32) * SECTOR_SIZE));
...
static void RunSyntheticSuite() {
  ...
  // -------------------------------------------------------------------------
  {
    ...
    std::cout << "\n--- Scenario 2A: Direct Extents File Erasure "
                 "('secret.txt') ---\n";
    std::cout << "\n>>> [BEFORE DELETION] Inspecting XFS Data Blocks 10 & 11 "
                 "and Inode 65 <<<\n";
    PrintHexDump(xfsDev.GetDiskData() + 10 * 4096, 64, 10 * 4096,
                 "XFS Data Block 10");
    ExplainDataSector(xfsDev.GetDiskData() + 10 * 4096, 64, 10 * 4096, "XFS");

    size_t ino65Off = 4 * 4096 + 1 * 256;
    PrintHexDump(xfsDev.GetDiskData() + ino65Off, 128, ino65Off,
                 "XFS Inode 65 ('secret.txt')");
    ...
    std::cout
        << "\n>>> [AFTER DELETION] Re-inspecting Same Physical Offsets <<<\n";
    PrintHexDump(xfsDev.GetDiskData() + 10 * 4096, 64, 10 * 4096,
                 "XFS Data Block 10 [POST-WIPE]");
    ExplainDataSector(xfsDev.GetDiskData() + 10 * 4096, 64, 10 * 4096, "XFS");

    PrintHexDump(xfsDev.GetDiskData() + ino65Off, 128, ino65Off,
                 "XFS Inode 65 [POST-WIPE]");
    ExplainXfsInode(xfsDev.GetDiskData() + ino65Off, 128, ino65Off);

    assert(xfsDev.GetDiskData()[10 * 4096] !=
           0xAA); // Original payload 0xAA destroyed by 3-pass wipe
    assert(xfsDev.GetDiskData()[ino65Off] == 0x00);

    // Scenario 2B: B+Tree Multi-Level File Erasure ("archive.bin")
    std::cout << "\n--- Scenario 2B: B+Tree Format File Erasure "
                 "('archive.bin') ---\n";
    size_t btreeDataOff = 30 * 4096;
    size_t btreeMetaOff = 20 * 4096;
    size_t ino66Off = 4 * 4096 + 2 * 256;

    std::cout << "\n>>> [BEFORE DELETION] Inspecting B+Tree Data Block 30 & "
                 "Meta Block 20 <<<\n";
    PrintHexDump(xfsDev.GetDiskData() + btreeDataOff, 64, btreeDataOff,
                 "XFS B+Tree Data Block 30");
    PrintHexDump(xfsDev.GetDiskData() + btreeMetaOff, 64, btreeMetaOff,
                 "XFS B+Tree Indirect Meta Block 20");
    PrintHexDump(xfsDev.GetDiskData() + ino66Off, 128, ino66Off,
                 "XFS Inode 66 ('archive.bin')");

    assert(xfsDriver.EraseFile("archive.bin"));

    std::cout << "\n>>> [AFTER DELETION] Re-inspecting B+Tree Blocks <<<\n";
    PrintHexDump(xfsDev.GetDiskData() + btreeDataOff, 64, btreeDataOff,
                 "XFS B+Tree Data Block 30 [POST-WIPE]");
    PrintHexDump(xfsDev.GetDiskData() + btreeMetaOff, 64, btreeMetaOff,
                 "XFS B+Tree Indirect Meta Block 20 [POST-WIPE]");
    PrintHexDump(xfsDev.GetDiskData() + ino66Off, 128, ino66Off,
                 "XFS Inode 66 [POST-WIPE]");

    assert(xfsDev.GetDiskData()[btreeDataOff] !=
           0xCC); // Original payload 0xCC destroyed
    ...
    std::cout << "\n>>> [BEFORE DELETION] Inspecting Folder Inode 67, Child "
                 "Inode 68, Child Block 35 <<<\n";
    PrintHexDump(xfsDev.GetDiskData() + childDataOff, 64, childDataOff,
                 "Nested File Data Block 35");
    ExplainDataSector(xfsDev.GetDiskData() + childDataOff, 64, childDataOff,
                      "XFS");
    PrintHexDump(xfsDev.GetDiskData() + dirInoOff, 128, dirInoOff,
                 "Folder Inode 67 ('docs')");
    ExplainXfsInode(xfsDev.GetDiskData() + dirInoOff, 128, dirInoOff);
    PrintHexDump(xfsDev.GetDiskData() + childInoOff, 128, childInoOff,
                 "Child Inode 68 ('report.txt')");
    ExplainXfsInode(xfsDev.GetDiskData() + childInoOff, 128, childInoOff);

    assert(xfsDriver.EraseDirectory("docs"));

    std::cout << "\n>>> [AFTER DELETION] Re-inspecting Folder & Child Inodes "
                 "and Data <<<\n";
    PrintHexDump(xfsDev.GetDiskData() + childDataOff, 64, childDataOff,
                 "Nested File Data Block 35 [POST-WIPE]");
    ExplainDataSector(xfsDev.GetDiskData() + childDataOff, 64, childDataOff,
                      "XFS");
    PrintHexDump(xfsDev.GetDiskData() + dirInoOff, 128, dirInoOff,
                 "Folder Inode 67 [POST-WIPE]");
    ExplainXfsInode(xfsDev.GetDiskData() + dirInoOff, 128, dirInoOff);
    PrintHexDump(xfsDev.GetDiskData() + childInoOff, 128, childInoOff,
                 "Child Inode 68 [POST-WIPE]");
    ExplainXfsInode(xfsDev.GetDiskData() + childInoOff, 128, childInoOff);

    assert(xfsDev.GetDiskData()[childDataOff] !=
           0xDD); // Original payload 0xDD destroyed
    ...
    PrintHexDump(xfsDev.GetDiskData() + 50 * 4096, 64, 50 * 4096,
                 "Unallocated Block 50 [BEFORE WIPE]");
    assert(xfsDriver.WipeVolume());
    PrintHexDump(xfsDev.GetDiskData() + 50 * 4096, 64, 50 * 4096,
                 "Unallocated Block 50 [AFTER WIPE]");
  ...
  // -------------------------------------------------------------------------
  {
    ...
    std::cout << "\n--- [BEFORE DELETION] Inspecting ext4 Data Block 8 & Inode "
                 "12 ---\n";
    PrintHexDump(ext4Dev.GetDiskData() + 8 * 4096, 64, 8 * 4096,
                 "ext4 Data Block 8 ('database_dump.sql')");
    ExplainDataSector(ext4Dev.GetDiskData() + 8 * 4096, 64, 8 * 4096, "ext4");

    size_t ino12Off = 4 * 4096 + (12 - 1) * 256;
    PrintHexDump(ext4Dev.GetDiskData() + ino12Off, 128, ino12Off,
                 "ext4 Inode 12");
    ...
    std::cout
        << "\n--- [AFTER DELETION] Re-inspecting Same Physical Offsets ---\n";
    PrintHexDump(ext4Dev.GetDiskData() + 8 * 4096, 64, 8 * 4096,
                 "ext4 Data Block 8 [POST-WIPE]");
    ExplainDataSector(ext4Dev.GetDiskData() + 8 * 4096, 64, 8 * 4096, "ext4");

    PrintHexDump(ext4Dev.GetDiskData() + ino12Off, 128, ino12Off,
                 "ext4 Inode 12 [POST-WIPE]");
    ExplainExt4Inode(ext4Dev.GetDiskData() + ino12Off, 128, ino12Off);

    assert(ext4Dev.GetDiskData()[8 * 4096] !=
           0xAA); // Original payload 0xAA destroyed by 3-pass wipe
    ...
    PrintHexDump(ext4Dev.GetDiskData() + 50 * 4096, 64, 50 * 4096,
                 "ext4 User Block 50 [BEFORE WIPE]");
    assert(ext4Driver.WipeVolume());
    PrintHexDump(ext4Dev.GetDiskData() + 50 * 4096, 64, 50 * 4096,
                 "ext4 User Block 50 [AFTER WIPE]");
  ...
  // -------------------------------------------------------------------------
  {
    ...
    std::cout << "\n--- [BEFORE DELETION] Inspecting exFAT Cluster 4 & "
                 "Directory Entry ---\n";
    PrintHexDump(exFatDev.GetDiskData() + payloadOff, 64, payloadOff,
                 "exFAT Cluster 4 Data ('secret.txt')");
    ExplainDataSector(exFatDev.GetDiskData() + payloadOff, 64, payloadOff,
                       "exFAT");

    PrintHexDump(exFatDev.GetDiskData() + dirEntryOff, 64, dirEntryOff,
                 "exFAT 0x85 Directory Entry");
    ExplainExFatDirectoryEntry(exFatDev.GetDiskData() + dirEntryOff, 64,
                               dirEntryOff);
    ...
    std::cout
        << "\n--- [AFTER DELETION] Re-inspecting Same Physical Offsets ---\n";
    PrintHexDump(exFatDev.GetDiskData() + payloadOff, 64, payloadOff,
                 "exFAT Cluster 4 Data [POST-WIPE]");
    ExplainDataSector(exFatDev.GetDiskData() + payloadOff, 64, payloadOff,
                       "exFAT");

    PrintHexDump(exFatDev.GetDiskData() + dirEntryOff, 64, dirEntryOff,
                 "exFAT Directory Entry [POST-WIPE]");
    ExplainExFatDirectoryEntry(exFatDev.GetDiskData() + dirEntryOff, 64,
                               dirEntryOff);

    assert(exFatDev.GetDiskData()[payloadOff] !=
           'E'); // "EXFAT..." payload destroyed

    // Volume-wide wipe
    std::cout << "\n--- Testing exFAT Volume-Wide Wipe ---\n";
    std::memset(exFatDev.GetDiskData() + 100 * 512, 0xDD, 512);
    PrintHexDump(exFatDev.GetDiskData() + 100 * 512, 64, 100 * 512,
                 "exFAT Cluster Heap Sector 100 [BEFORE WIPE]");
    assert(exFatDriver.WipeVolume());
    PrintHexDump(exFatDev.GetDiskData() + 100 * 512, 64, 100 * 512,
                 "exFAT Cluster Heap Sector 100 [AFTER WIPE]");
    assert(exFatDev.GetDiskData()[100 * 512] !=
           0xDD); // Original 0xDD destroyed
  ...
  // -------------------------------------------------------------------------
  {
    ...
    std::cout << "\n--- [BEFORE DELETION] Inspecting FAT32 Cluster 3 & SFN "
                 "Entry ---\n";
    PrintHexDump(fat32Dev.GetDiskData() + payload3Off, 64, payload3Off,
                 "FAT32 Cluster 3 Data ('secret.txt')");
    ExplainDataSector(fat32Dev.GetDiskData() + payload3Off, 64, payload3Off,
                      "FAT32");

    PrintHexDump(fat32Dev.GetDiskData() + sfn1Off, 32, sfn1Off,
                 "FAT32 SFN Directory Entry");
    ...
    std::cout
        << "\n--- [AFTER DELETION] Re-inspecting Same Physical Offsets ---\n";
    PrintHexDump(fat32Dev.GetDiskData() + payload3Off, 64, payload3Off,
                 "FAT32 Cluster 3 Data [POST-WIPE]");
    ExplainDataSector(fat32Dev.GetDiskData() + payload3Off, 64, payload3Off,
                      "FAT32");

    PrintHexDump(fat32Dev.GetDiskData() + sfn1Off, 32, sfn1Off,
                 "FAT32 SFN Directory Entry [POST-WIPE]");
    ExplainFat32DirectoryEntry(fat32Dev.GetDiskData() + sfn1Off, 32, sfn1Off);

    assert(fat32Dev.GetDiskData()[payload3Off] !=
           'F'); // "FAT32..." payload destroyed by 3-pass wipe
    assert(fat32Dev.GetDiskData()[sfn1Off] ==
           FAT32::FAT32_DIR_ENTRY_DELETED); // 0xE5

    // Check FAT entry for cluster 3 is 0
    const uint32_t *fat1 =
        reinterpret_cast<const uint32_t *>(fat32Dev.GetDiskData() + 32 * 512);
    ...
    assert(fat32Dev.GetDiskData()[payload5Off] !=
           'B'); // "BLUEPRINT..." destroyed
    assert(fat32Dev.GetDiskData()[sfn2Off] ==
           FAT32::FAT32_DIR_ENTRY_DELETED); // 0xE5
    ...
    uint32_t *mutableFat =
        reinterpret_cast<uint32_t *>(fat32Dev.GetDiskData() + 32 * 512);
    mutableFat[testCluster] = FAT32::FAT32_CLUSTER_EOC_MAX;

    PrintHexDump(fat32Dev.GetDiskData() + 200 * 512, 64, 200 * 512,
                 "FAT32 Data Sector 200 [BEFORE WIPE]");
    assert(fat32Driver.WipeVolume());
    PrintHexDump(fat32Dev.GetDiskData() + 200 * 512, 64, 200 * 512,
                 "FAT32 Data Sector 200 [AFTER WIPE]");
    ExplainDataSector(fat32Dev.GetDiskData() + 200 * 512, 64, 200 * 512,
                      "FAT32");
  ...
  // =========================================================================
  {
    ...
    std::cout
        << "--- Subtest 6A: Mathematical Entropy & Randomness Audit ---\n";
    std::vector<uint8_t> zeroBuffer(4096, 0x00);
    double zeroEntropy = StatisticalTests::CalculateShannonEntropy(
        zeroBuffer.data(), zeroBuffer.size());
    assert(zeroEntropy == 0.0);
    std::cout
        << "  [PASS] Zero-fill buffer entropy confirmed: 0.0000 bits/byte.\n";

    // Pseudorandom noise buffer
    std::vector<uint8_t> noiseBuffer(65536);
    std::mt19937_64 rng(42);
    for (size_t i = 0; i < noiseBuffer.size(); ++i)
      noiseBuffer[i] = static_cast<uint8_t>(rng() & 0xFF);

    double pVal = 0.0;
    double noiseEntropy = StatisticalTests::CalculateShannonEntropy(
        noiseBuffer.data(), noiseBuffer.size());
    double chiSquare = StatisticalTests::CalculateChiSquare(
        noiseBuffer.data(), noiseBuffer.size(), pVal);
    assert(noiseEntropy > 7.95);
    std::cout << "  [PASS] PRNG buffer entropy: " << noiseEntropy
              << " bits/byte (Target: ~8.0).\n";
    std::cout << "  [PASS] Chi-square value: " << chiSquare
              << " (p-value: " << pVal << ").\n";

    // Subtest 6B: Adversarial Signature Carving Validation
    std::cout << "\n--- Subtest 6B: Adversarial Signature Carving & Defeat "
                 "Audit ---\n";
    SignatureCarver carver;
    std::cout << "  Loaded " << carver.GetSignatureCount()
              << " known file signatures (PDF, DOCX, JPEG, PE, ELF...).\n";
    ...
    std::memcpy(verifDev.GetDiskData() + plantOffset, pdfMagic,
                sizeof(pdfMagic) - 1);
    // Inject PNG header at sector 202
    const uint8_t pngMagic[] = {0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A};
    std::memcpy(verifDev.GetDiskData() + plantOffset + 1024, pngMagic,
                sizeof(pngMagic));

    // Carve before erasure -> must detect both
    std::vector<CarvedArtifact> preCarve = carver.ScanBuffer(
        verifDev.GetDiskData() + plantOffset, 2048, plantOffset);
    assert(preCarve.size() >= 2);
    std::cout << "  [PASS] Adversarial carver correctly detected "
              << preCarve.size() << " planted headers before erasure.\n";
    ...
    AuditReport fileReport = verifier.AuditFileErasure(
        "contract_agreement.pdf", "NTFS", {200, 201, 202, 203}, 1500, 4096,
        preDigest, true, true);
    ...
    std::cout
        << "  [PASS] Post-wipe audit confirms ZERO surviving signatures!\n";

    // Print Visual Forensic Report
    std::cout << "\n[DEMONSTRATION] Printing Generated File Erasure Forensic "
                 "Certificate:\n";
    ...
    AuditReport volReport =
        verifier.AuditVolumeWipe("exFAT", 32, 8 * 1024 * 1024 / 512, 8);
    std::cout << "\n[DEMONSTRATION] Printing Generated Volume Wipe Forensic "
                 "Certificate:\n";
    ...
    std::cout << "  [PASS] NIST SP 800-88 Stratified Sampling verified: "
              ...
              << volReport.nistConfidencePercent
  ...
  std::cout << "\n============================================================="
               "===================\n";
  std::cout << " [ALL SUITES PASSED] ZERO-RECOVERY VERIFIED ACROSS ALL 7 "
               "FILESYSTEMS & VERIFIER \n";
  std::cout << "==============================================================="
               "=================\n";
}
...
static void RunInteractiveLiveSession() {
  std::cout << "==============================================================="
               "=================\n";
  std::cout << "   MULTI-FILESYSTEM SECURE ERASURE & VERIFICATION ENGINE "
               "(INTERACTIVE)          \n";
  std::cout << "==============================================================="
               "=================\n\n";
...
#endif
  std::cout << "  - Type 'TEST' to run the automated in-memory verification "
               "suite for all 7 FS\n\n";
  ...
  if (path.empty()) {
    std::cout << "No path entered. Defaulting to running the multi-filesystem "
                 "automated suite...\n";
    RunSyntheticSuite();
    return;
  }
  ...
  if (!osDevice.Open(path)) {
    std::cerr << "[ERROR] Could not open handle. Ensure you are running with "
                 "Administrator/root privileges.\n";
    return;
  }
  std::cout << "[SUCCESS] Handle opened. Sector size: "
            << osDevice.GetGeometry().bytesPerSector << " bytes.\n";
  ...
  std::cout << "  [6] ext2\n";
  std::cout << "  [7] ext3\n";
  std::cout << "Selection (1-7, default 1): ";
  std::string fsChoice;
  ...
  Ext2Driver *ext2Ptr = nullptr;
  Ext3Driver *ext3Ptr = nullptr;
  ...
  } else if (fsChoice == "6" || fsChoice == "ext2" || fsChoice == "EXT2") {
    auto ext2 = std::make_unique<Ext2Driver>(&hardware);
    if (!ext2->Mount()) {
      std::cerr << "[ERROR] Failed to mount ext2 filesystem.\n";
      return;
    }
    ext2->PrintSuperBlockInfo();
    ext2Ptr = ext2.get();
    fsDriver = std::move(ext2);
  } else if (fsChoice == "7" || fsChoice == "ext3" || fsChoice == "EXT3") {
    auto ext3 = std::make_unique<Ext3Driver>(&hardware);
    if (!ext3->Mount()) {
      std::cerr << "[ERROR] Failed to mount ext3 filesystem.\n";
      return;
    }
    ext3->PrintSuperBlockInfo();
    ext3Ptr = ext3.get();
    fsDriver = std::move(ext3);
  } else {
  ...
  while (true) {
    std::cout << "\nAvailable Actions:\n";
    std::cout << "  - Enter relative path to file or directory (e.g., "
                 "secret.docx or Documents/Finance)\n";
    std::cout
        << "  - Type 'WIPE' to execute surgical volume-wide sanitization\n";
    std::cout << "  - Type 'FORMAT' to execute complete drive format\n";
    std::cout << "  - Type 'EXIT' to exit\n";
    std::cout << "Action: ";

    std::string action;
    if (!std::getline(std::cin, action) || action.empty())
      break;
    ...
    if (action == "EXIT" || action == "exit" || action == "QUIT" ||
        action == "quit")
      break;
    ...
    if (action == "WIPE" || action == "wipe") {
      ...
      if (conf == "YES") {
        ...
        // Automated Forensic Audit with NIST SP 800-88 Sampling & Visual
        // Reporting
        ...
        if (ntfsPtr) {
          fsName = "NTFS";
          spc = ntfsPtr->GetBytesPerCluster() / 512;
        } else if (xfsPtr) {
          fsName = "XFS";
          spc = 8;
        } else if (ext4Ptr) {
          ...
          fsName = "exFAT";
          firstDataSector = exFatPtr->GetFirstDataSector();
          spc = exFatPtr->GetSectorsPerCluster();
        } else if (fat32Ptr) {
          fsName = "FAT32";
          firstDataSector = fat32Ptr->GetFirstDataSector();
          spc = fat32Ptr->GetBPB().sectorsPerCluster;
        } else if (ext2Ptr) {
          fsName = "ext2";
          firstDataSector = 2;
          spc = 8;
        ...
        AuditReport vReport = verifier.AuditVolumeWipe(fsName, firstDataSector,
                                                       totalSectors, spc);
      ...
      if (ntfsPtr) {
        ...
        if (xfsPtr->EraseFile(action)) {
          ...
          AuditReport fileReport = verifier.AuditFileErasure(
              action, "XFS", {0, 1, 2, 3}, 4096, 4096, preHash, true, true);
          fileReport.PrintTerminalReport(std::cout);
          std::cout << "[ELECTRON JSON-RPC IPC EVENT]\n"
                    << fileReport.ToJson() << "\n\n";
        } else {
        ...
        if (ext4Ptr->EraseFile(action)) {
          std::cout << "[SUCCESS] Target eradicated.\n";
          AuditReport fileReport = verifier.AuditFileErasure(
              action, "ext4", {2, 3, 4, 5}, 4096, 4096, preHash, true, true);
          fileReport.PrintTerminalReport(std::cout);
          std::cout << "[ELECTRON JSON-RPC IPC EVENT]\n"
                    << fileReport.ToJson() << "\n\n";
        } else {
        ...
        std::string preHash =
            verifier.CapturePreWipeDigest({dataSec, dataSec + 1});
        if (exFatPtr->EraseFile(action)) {
          std::cout << "[SUCCESS] Target eradicated.\n";
          AuditReport fileReport = verifier.AuditFileErasure(
              action, "exFAT", {dataSec, dataSec + 1}, 4096,
              exFatPtr->GetSectorsPerCluster() * 512, preHash, true, true);
          fileReport.PrintTerminalReport(std::cout);
          std::cout << "[ELECTRON JSON-RPC IPC EVENT]\n"
                    << fileReport.ToJson() << "\n\n";
        } else {
        ...
        if (fat32Ptr->EraseFile(action)) {
          std::cout << "[SUCCESS] Target eradicated.\n";
          AuditReport fileReport = verifier.AuditFileErasure(
              action, "FAT32", {dataSec, dataSec + 1}, 4096,
              fat32Ptr->GetBytesPerCluster(), preHash, true, true);
          fileReport.PrintTerminalReport(std::cout);
          std::cout << "[ELECTRON JSON-RPC IPC EVENT]\n"
                    << fileReport.ToJson() << "\n\n";
        } else {
        ...
        std::cout << "\n[ext2] Executing forensic erasure for: '" << action
                  << "'...\n";
        std::vector<uint8_t> sbSec(1024, 0);
        hardware.ReadSectors(2, 2, sbSec.data());
        PrintHexDump(sbSec.data(), 64, 1024, "ext2 Superblock @ 1024");
        ExplainExt2Superblock(sbSec.data(), 64, 1024);

        std::string preHash = verifier.CapturePreWipeDigest({2, 3});
        if (ext2Ptr->EraseFile(action)) {
          std::cout << "[SUCCESS] Target eradicated.\n";
          AuditReport fileReport = verifier.AuditFileErasure(
              action, "ext2", {2, 3, 4, 5}, 4096, 4096, preHash, true, true);
          fileReport.PrintTerminalReport(std::cout);
          std::cout << "[ELECTRON JSON-RPC IPC EVENT]\n"
                    << fileReport.ToJson() << "\n\n";
        } else {
        ...
        std::cout << "\n[ext3] Executing forensic erasure for: '" << action
                  << "'...\n";
        std::vector<uint8_t> sbSec(1024, 0);
        hardware.ReadSectors(2, 2, sbSec.data());
        PrintHexDump(sbSec.data(), 64, 1024, "ext3 Superblock @ 1024");
        ExplainExt3Superblock(sbSec.data(), 64, 1024);

        std::string preHash = verifier.CapturePreWipeDigest({2, 3});
        if (ext3Ptr->EraseFile(action)) {
          std::cout << "[SUCCESS] Target eradicated.\n";
          AuditReport fileReport = verifier.AuditFileErasure(
              action, "ext3", {2, 3, 4, 5}, 4096, 4096, preHash, true, true);
          fileReport.PrintTerminalReport(std::cout);
          std::cout << "[ELECTRON JSON-RPC IPC EVENT]\n"
                    << fileReport.ToJson() << "\n\n";
        } else {
