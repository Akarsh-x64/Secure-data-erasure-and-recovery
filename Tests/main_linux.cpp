...
static void RunInteractiveLiveSession() {
  ...
  OS::LinuxStorageDevice osDevice;
  std::cout << "\nOpening handle to: " << path
            << " with O_DIRECT | O_SYNC ...\n";
  if (!osDevice.Open(path)) {
    std::cerr << "[ERROR] Could not open handle to " << path
              << ". Ensure you are running with sudo/root privileges.\n";
    return;
  }
  std::cout << "[SUCCESS] Device opened. Sector size: "
            << osDevice.GetGeometry().bytesPerSector << " bytes.\n";

  // 2. Hardware Controller
  HDDController hardware(&osDevice);

  std::cout << "Attempting volume lock/dismount to bypass OS block cache...\n";
  if (osDevice.LockVolume())
    std::cout << "  -> Block device exclusively locked via flock().\n";
  if (osDevice.DismountVolume())
    std::cout << "  -> Kernel block buffers flushed via BLKFLSBUF.\n";
