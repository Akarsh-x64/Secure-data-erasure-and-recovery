...
#include <sstream>
#include <string>
#include <unistd.h> // POSIX geteuid() for root check
#include <vector>
...
static void ExplainDataSector(const uint8_t *data, size_t size,
  ...
  if (allZero) {
    std::cout
        << "  * Status: ALL BYTES ZEROED (0x00) - Sanitized / Free Cluster\n";
  } else {
    std::cout
        << "  * Status: ACTIVE DATA DETECTED - Valid File Payload Present\n";
  }
}

static void ExplainNtfsBootSector(const uint8_t *data, size_t size,
                                  uint64_t physicalOffset) {
  if (size < sizeof(NTFS::NtfsBootSector))
    return;
  const auto *vbr = reinterpret_cast<const NTFS::NtfsBootSector *>(data);
  ...
  std::cout << "  * Offset +0x30..+0x37 [MFT Start LCN]: Cluster "
            << vbr->mftStartLCN << " (Sector "
            << (vbr->mftStartLCN * vbr->sectorsPerCluster) << ")\n";
...
static void RunSyntheticSuite() {
  ...
  std::cout << "\n============================================================="
               "===================\n";
  std::cout << " [ALL SUITES PASSED] ZERO-RECOVERY VERIFIED ACROSS ALL 5 "
...
// ============================================================================
// Interactive Live Device & File System Testing Mode (Linux Native)
// ============================================================================
...
static void RunInteractiveLiveSession() {
  ...
  if (geteuid() != 0) {
    std::cout << "[SECURITY WARNING] Not running as root (EUID != 0).\n";
    std::cout << "  Direct block device I/O (e.g. /dev/sdb) typically requires "
                 "root privileges (sudo).\n\n";
  }

  std::cout << "Supported Targets:\n";
  std::cout << "  - Physical Block Device: /dev/sdb, /dev/nvme0n1\n";
  std::cout << "  - Mounted Volume Partition: /dev/sdb1, /dev/nvme0n1p1\n";
  std::cout << "  - Loop Device / Raw Image: /dev/loop0, /path/to/disk.img\n";
  ...
  // 1. Open Device via OS Layer (Linux Native)
  ...
  // 3. User Selects Active Filesystem Driver
  std::cout << "\nSelect Filesystem Driver to initialize:\n";
  std::cout << "  1. NTFS\n";
  std::cout << "  2. XFS\n";
  std::cout << "  3. ext4\n";
  std::cout << "  4. exFAT\n";
  ...
  if (success) {
    std::cout
        << "[System] Triggering native OS filesystem consistency check...\n";
    osDevice.Close();
    ...
    if (result == 0) {
      std::cout << "[SUCCESS] Native OS metadata check completed cleanly. "
                   "Filesystem is fully consistent.\n";
    } else {
      std::cout << "[WARNING] Filesystem check completed with exit code: "
                << result << "\n";
    }
  }
