...
static void ExplainDataSector(const uint8_t* data, size_t size, uint64_t physicalOffset, const std::string& fsName) {
    ...
    if (allZero) {
        std::cout << "  * Status: ALL BYTES ZEROED (0x00) - Sanitized / Free Cluster\n";
    } else {
        std::cout << "  * Status: ACTIVE DATA DETECTED - Valid File Payload Present\n";
    }
}

static void ExplainNtfsBootSector(const uint8_t* data, size_t size, uint64_t physicalOffset) {
    ...
    std::cout << "  * Offset +0x0B..+0x0C [Bytes/Sector]:  " << vbr->bytesPerSector << " bytes\n";
    ...
    std::cout << "  * Offset +0x30..+0x37 [MFT Start LCN]: Cluster " << vbr->mftStartLCN
              << " (Sector " << (vbr->mftStartLCN * vbr->sectorsPerCluster) << ")\n";
...
static void RunSyntheticSuite() {
    std::cout << "\n================================================================================\n";
    std::cout << "        RUNNING COMPREHENSIVE AUTOMATED VERIFICATION SUITE (ALL 5 FS)           \n";
    std::cout << "================================================================================\n";
...
static void RunInteractiveLiveSession() {
    std::cout << "================================================================================\n";
    std::cout << "   MULTI-FILESYSTEM SECURE ERASURE & VERIFICATION ENGINE (LINUX NATIVE)         \n";
    std::cout << "================================================================================\n\n";

    // Check root / sudo privileges
    if (geteuid() != 0) {
        std::cout << "[SECURITY WARNING] Not running as root (EUID != 0).\n";
        std::cout << "  Direct block device I/O (e.g. /dev/sdb) typically requires root privileges (sudo).\n\n";
    }
    ...
    std::cout << "\nOpening handle to: " << path << " with O_DIRECT | O_SYNC ...\n";
    if (!osDevice.Open(path)) {
        std::cerr << "[ERROR] Could not open handle to " << path << ". Ensure you are running with sudo/root privileges.\n";
        return;
    }
    std::cout << "[SUCCESS] Device opened. Sector size: " << osDevice.GetGeometry().bytesPerSector << " bytes.\n";

    // 2. Hardware Controller
    HDDController hardware(&osDevice);

    std::cout << "Attempting volume lock/dismount to bypass OS block cache...\n";
    if (osDevice.LockVolume()) std::cout << "  -> Block device exclusively locked via flock().\n";
    if (osDevice.DismountVolume()) std::cout << "  -> Kernel block buffers flushed via BLKFLSBUF.\n";
