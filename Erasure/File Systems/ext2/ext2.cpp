...
namespace FileSystems {
    ...
    Ext2Driver::Ext2Driver(Core::IHardwareController* hardware)
        : m_hardware(hardware),
          m_block_size(0),
          m_inode_size(0),
          m_blocksPerGroup(0),
    ...
    uint64_t Ext2Driver::InodeToSector(uint32_t inode) const {
    ...
    uint64_t Ext2Driver::BlockToSector(uint32_t blockNum) const {
        uint32_t bytesPerSector = m_hardware->GetGeometry().bytesPerSector;
        if (bytesPerSector == 0) return 0;
    ...
    bool Ext2Driver::Mount() {
        ...
        std::memcpy(&m_superblock, &sectorBuffer[offsetInSector], sizeof(Ext2SuperBlock));

        // Ext2 Verification using Magic Number (0xEF53)
    ...
    bool Ext2Driver::GetGroupDescriptor(uint32_t groupNumber, Ext2GroupDescriptor& outDescriptor) const {
        ...
        uint64_t exactDescriptorOffset = tableBaseByteOffset + (static_cast<uint64_t>(groupNumber) * sizeof(Ext2GroupDescriptor));
        uint32_t bytesPerSector = m_hardware->GetGeometry().bytesPerSector;
        ...
        std::memcpy(&outDescriptor, &sectorBuffer[offsetInsideSector], sizeof(Ext2GroupDescriptor));
        return true;
    }

    bool Ext2Driver::ReadInode(uint32_t inodeNum, Ext2Inode& outInode) const {
        ...
        std::memcpy(&outInode, &sectorBuffer[offsetInsideSector], sizeof(Ext2Inode));
        return true;
    }

    bool Ext2Driver::WriteInode(uint32_t inodeNum, const Ext2Inode& inode) {
        if (inodeNum == 0 || inodeNum > m_superblock.inode_count || m_inodesPerGroup == 0) {
            return false;
        }
        ...
        std::memcpy(&sectorBuffer[offsetInsideSector], &inode, sizeof(Ext2Inode));
        return m_hardware->WriteSectors(targetSector, 1, sectorBuffer.data());
    }

    bool Ext2Driver::ClearBlockBitMapBit(uint32_t blockNum) {
    ...
    bool Ext2Driver::ClearInodeBitMapBit(uint32_t inodeNum) {
        if (inodeNum == 0 || inodeNum > m_superblock.inode_count || m_inodesPerGroup == 0) {
            return false;
        }
    ...
    std::vector<std::string> Ext2Driver::TokenizePath(const std::string& path) const {
    ...
    std::vector<uint32_t> Ext2Driver::ReadBlockAsPointers(uint32_t blockNum) const {
    ...
    std::vector<uint32_t> Ext2Driver::GetInodeBlocks(const Ext2Inode& inode) const {
    ...
    std::vector<uint32_t> Ext2Driver::GetInodeBlocks(uint32_t inodeNum) const {
        Ext2Inode inode;
    ...
    std::vector<uint32_t> Ext2Driver::GetAllInodeBlocksIncludingIndirect(const Ext2Inode& inode) const {
    ...
    Ext2Driver::SearchResult Ext2Driver::FindEntryInDirectory(const std::vector<uint32_t>& dirBlocks, const std::string& targetName) const {
        ...
        for (uint32_t block : dirBlocks) {
            ...
            while (offset < m_block_size) {
                if (offset + sizeof(Ext2DirEntry) > m_block_size) break;

                Ext2DirEntry* entry = reinterpret_cast<Ext2DirEntry*>(&blockBuffer[offset]);
    ...
    bool Ext2Driver::EraseFile(const std::string& relativePath) {
        if (m_block_size == 0) return false;

        std::vector<std::string> pathTokens = TokenizePath(relativePath);
        if (pathTokens.empty()) {
            std::cout << "[Ext2] ERROR: Invalid path provided.\n";
            return false;
        }

        std::cout << "\n--- Initiating Ext2 Secure Erase for: " << relativePath << " ---\n";

        uint32_t currentInodeNum = EXT2_ROOT_INO;
        ...
        for (size_t i = 0; i < pathTokens.size(); ++i) {
            ...
            std::cout << "[Ext2 Parser] Searching for '" << token << "' in directory inode " << currentInodeNum << "...\n";

            std::vector<uint32_t> dirBlocks = GetInodeBlocks(currentInodeNum);
            searchRes = FindEntryInDirectory(dirBlocks, token);

            if (!searchRes.found) {
                std::cout << "[Ext2 Parser] ERROR: '" << token << "' not found!\n";
                return false;
            }

            if (!isLastToken) {
                if (!searchRes.isDirectory) {
                    std::cout << "[Ext2 Parser] ERROR: '" << token << "' is a file, not a directory!\n";
                    return false;
                }
        ...
        std::cout << "[Ext2] Target file found with Inode: " << targetInode << "\n";

        Ext2Inode fileInode;
        if (!ReadInode(targetInode, fileInode)) {
            std::cout << "[Ext2] ERROR: Failed to read target inode.\n";
            return false;
        }
        ...
        std::cout << "[Ext2] Securely wiping " << blocksToWipe.size() << " data/indirect blocks...\n";
        ...
        std::cout << "[Ext2] Zeroing out Inode metadata and updating Inode Bitmap...\n";
        Ext2Inode zeroInode;
        ...
        std::cout << "[Ext2] Unlinking directory entry from parent block " << searchRes.dirBlockNumber << "...\n";
        ...
        if (m_hardware->ReadSectors(dirSector, sectorsPerBlock, dirBlockBuffer.data())) {
            Ext2DirEntry* targetEntry = reinterpret_cast<Ext2DirEntry*>(&dirBlockBuffer[searchRes.entryOffset]);
            if (searchRes.previousRecLen > 0) {
                Ext2DirEntry* prevEntry = reinterpret_cast<Ext2DirEntry*>(&dirBlockBuffer[searchRes.previousEntryOffset]);
        ...
        std::cout << "=== Ext2 File Securely Wiped ===\n";
        return true;
    }

    bool Ext2Driver::WipeVolume() {
        if (m_block_size == 0) return false;

        std::cout << "\n=== Initiating Full Ext2 Volume Wipe ===\n";
        ...
        std::cout << "=== Ext2 Volume Securely Wiped! ===\n";
        return true;
    }

    void Ext2Driver::PrintSuperBlockInfo() const {
        if (m_superblock.s_magic != 0xEF53) {
            std::cout << "Drive is not mounted or not a valid Ext2 filesystem!\n";
            return;
        }

        std::cout << "\n================ Ext2 SuperBlock Info ================\n";
