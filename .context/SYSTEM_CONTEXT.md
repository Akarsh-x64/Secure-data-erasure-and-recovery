...
# Phase 11 — Python Flask & Socket.IO Backend Architecture (`backend/main.py`)

## Overview & Background

To bridge the high-level graphical user interface with the native C++ pybind11 modules, Phase 11 implements a high-performance, asynchronous REST and WebSocket API server located in `backend/main.py`.

### 1. Key Architectural Features
- **Elevation Supervisor (`ensure_admin()`)**:
  - Automatically queries `ctypes.windll.shell32.IsUserAnAdmin()` on Windows.
  - Automatically re-launches with elevation via `ShellExecuteW(None, "runas", ...)` if unprivileged.
  - Enforces `geteuid() == 0` on Linux.
- **Data Contracts & Pydantic Validation**:
  - `StorageDevice` & `DeviceCapabilities`: Reports hardware health, bus type (`NVMe`, `SATA`, `USB`), capacity, sector size, and capabilities (`fileErase`, `driveErase`, `cryptoErase`, `trim`).
  - `FileNode`: Hierarchical tree representation of mounted volumes, active files, directories, and deleted/recovered artifacts.
  - `ProgressEvent`: Real-time telemetry packet delivering `phase`, `percent`, `throughputMBps`, `etaSeconds`, and timestamps.
- **REST Endpoints**:
  - `GET /api/v1/devices`: Discovers physical storage devices.
  - `GET /api/v1/file-node`: Enumerates filesystem nodes.
  - `POST /api/v1/erase/files/validate` & `POST /api/v1/erase/files`: Accepts targeted files, validates idempotency keys and `CONFIRM_ERASE` tokens, returning `202 Accepted` with `operationId`.
  - `POST /api/v1/erase/drives/validate` & `POST /api/v1/erase/drives`: Accepts whole drive wipe targets requiring `CONFIRM_WIPE`.
- **Asynchronous Worker Threads & Telemetry**:
  - Runs operations in dedicated background threads (`background_file_erase_worker` and `background_drive_erase_worker`).
  - Streams real-time progress events over WebSockets (`socketio.emit('progress', event)`).
- **Automated OS Filesystem Repair (`repair_filesystem`)**:
  - Automatically invokes native repair tools after low-level disk operations to guarantee zero filesystem corruptions:
    - Windows: `chkdsk <volume> /f /x`
    - Linux: `_externals/e2fsck -y -f`, `fsck.exfat -a`, `fsck.fat -a`, `ntfsfix -d`, or `xfs_repair`.

---

# Phase 12 — Electron 44 & React 19 Frameless Desktop UI (`frontend/`)

## Overview & Architecture

Phase 12 implements the production desktop interface using **Electron 44**, **electron-vite 5**, **React 19**, **TypeScript 5.9**, and **Tailwind CSS 3.4**.

### 1. Main Process & Preload ContextBridge
- **Frameless Window**: Configured with `frame: false` for a bespoke dark-slate UI, custom window minimize/maximize/close buttons, and custom titlebar.
- **Preload API (`src/preload/index.ts`)**: Enforces `contextIsolation = true` and `nodeIntegration = false`. Exposes `window.api` with `eraseFiles`, `minimizeWindow`, `maximizeWindow`, and `closeWindow`.

### 2. Operational Modules
1. **File & Folder Erasure Tab (`FileEraseTab.tsx`)**:
   - `FileSystemTree.tsx`: Interactive tree with folder collapse/expand, status icons (deleted, active, corrupted), and selective item queueing.
   - `SelectedFilesPanel.tsx`: Displays queued files with cluster geometry and filesystem tags.
   - `EraseConfigPanel.tsx`: Multi-pass configuration (1-7 passes, DoD 5220.22-M, slack space wiping, metadata clearing).
   - `EraseConfirmDialog.tsx`: Safety confirmation dialog.
2. **Whole Drive Erasure Tab (`DriveEraseTab.tsx`)**:
   - Lists physical storage devices with real-time health and bus type indicators.
...
4. **Audit Logs Tab (`AuditLogsTabs.tsx`)**:
   ...
   - Exportable Sanitization Certificates (PDF / JSON).

---

# Phase 13 — Dedicated RAM-Disk LiveBoot OS (`toram`)

## Overview & The System Drive Solution

Standard desktop operating systems cannot sanitize their own active boot drive (`C:\` on Windows or `/` on Linux) because the kernel holds active locks on system files and the pagefile.

Phase 13 establishes the **LiveBoot OS Architecture**:
- A custom, hardened Linux distribution (Alpine/Debian) compiled with `toram` / `copytoram`.
- Boots from USB into host RAM (`tmpfs`), allowing the USB stick to be safely unplugged.
- The host internal drives (`/dev/nvme0n1`, `/dev/sda`) remain 100% offline and unmounted.
- SanitizeX executes unrestricted bare-metal NVMe Sanitize, ATA Secure Erase, and DoD 3-pass overwriting directly on the primary host drives.

---

## Master Documentation Suite

For exhaustive, modular references for each component, consult the dedicated manuals in `.context/`:
1. [00_INDEX.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/00_INDEX.md) — Master Navigation & Architecture Map
2. [01_SYSTEM_ARCHITECTURE.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/01_SYSTEM_ARCHITECTURE.md) — Core Philosophy & 3-Layer Decoupled Design
3. [02_STORAGE_AND_HARDWARE.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/02_STORAGE_AND_HARDWARE.md) — Storage Pipes & DoD 3-Pass Overwriter
4. [03_FILESYSTEMS_ERASURE_ENGINE.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/03_FILESYSTEMS_ERASURE_ENGINE.md) — Multi-Filesystem Surgical Deletion
5. [04_FORENSIC_RECOVERY_ENGINE.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/04_FORENSIC_RECOVERY_ENGINE.md) — Read-Only Forensic Recovery & TSK Bridge
6. [05_VERIFICATION_AND_AUDIT.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/05_VERIFICATION_AND_AUDIT.md) — Statistical Suite & 120+ Signature Carver
7. [06_PYBIND11_NATIVE_MODULES.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/06_PYBIND11_NATIVE_MODULES.md) — pybind11 C++ Extensions Subsystem
8. [07_BACKEND_AND_IPC.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/07_BACKEND_AND_IPC.md) — Python Flask + Socket.IO Backend Architecture
9. [08_FRONTEND_ELECTRON_REACT.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/08_FRONTEND_ELECTRON_REACT.md) — Electron Desktop Frontend & React UI
10. [09_SHIPPING_AND_LIVEBOOT.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/09_SHIPPING_AND_LIVEBOOT.md) — The 3 Shipping Formats & RAM-Disk LiveBoot
11. [10_BUILD_AND_TEST_GUIDE.md](file:///c:/Users/Sudhit/Documents/Study%20Material/Projects/SIH%20v2/.context/10_BUILD_AND_TEST_GUIDE.md) — Compilation, Building & Testing Manual
